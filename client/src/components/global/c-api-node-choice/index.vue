<template>
  <div>
    <b-modal
      v-model="apiNodeModal"
      hide-footer
      centered
      title="Choose API Node"
    >
      <div class="node-list">
        <b-row
          v-for="node in apiNodeList"
          :key="node.ip"
          class="node-item"
        ...
        >
          ...
          <b-col cols="4" class="ping"> Ping time {{ node.ping }} ms </b-col>

          <b-col cols="2" class="icon">
            <i class="fa fa-chain fa-2x" />
          </b-col>
        </b-row>
      </div>
      ...
      <b-row align-h="end" class="mt-4">
        <b-col cols="6">
          <b-button type="button" variant="default" block @click="closeModal">
            Close
          </b-button>
        </b-col>
      </b-row>
    ...
    <transition name="fade">
      <div v-if="isConnecting" class="loading">
        ...
        <i class="fa fa-3x fa-cog fa-spin" />
